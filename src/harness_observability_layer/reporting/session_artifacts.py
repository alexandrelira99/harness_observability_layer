...
from harness_observability_layer.integrations.codex_jsonl import normalize_codex_jsonl_file
from harness_observability_layer.integrations.claude_code_jsonl import normalize_claude_code_jsonl_file
...
DEFAULT_ARTIFACTS_DIRNAME = "hol-artifacts"
LEGACY_ARTIFACTS_DIRNAME = "artifacts"
...
def ensure_project_artifact_dirs(project_root: str | Path) -> Dict[str, Path]:
    ...
    artifacts_root = root / DEFAULT_ARTIFACTS_DIRNAME
    sessions_root = artifacts_root / "sessions"
    live_runs_root = artifacts_root / "live_runs"
    legacy_artifacts_root = root / LEGACY_ARTIFACTS_DIRNAME
    legacy_sessions_root = legacy_artifacts_root / "sessions"
    legacy_live_runs_root = legacy_artifacts_root / "live_runs"
    ...
    return {
        ...
        "legacy_artifacts_root": legacy_artifacts_root,
...
def refresh_sessions_index(project_root: str | Path) -> Path:
    ...
    if legacy_sessions_root.exists():
        source_roots.append(legacy_sessions_root)
    ...
    for source_root in source_roots:
        for path in source_root.iterdir():
            ...
            if resolved in seen_session_dirs:
                continue
            seen_session_dirs.add(resolved)
            all_session_dirs.append(path)
    for session_dir in sorted(all_session_dirs, key=lambda p: p.name, reverse=True):
        ...
        try:
            ...
            continue
        metadata = derive_session_metadata(session_dir.name, load_events(normalized_path), include_prompt_excerpt=True) if normalized_path.exists() else {}
...
def import_codex_session_to_dir(
    ...
    *,
    ...
    metadata = derive_session_metadata(session_dir.name, events, include_prompt_excerpt=True)
