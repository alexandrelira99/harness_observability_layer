...
def compute_metrics(events: List[Dict[str, Any]], *, resolve_file_stats: bool = True) -> Dict[str, Any]:
    ...
    top_tool_count = 0
    if tool_counts:
        top_tool_name, top_tool_count = sorted(tool_counts.items(), key=lambda item: (-item[1], item[0]))[0]
    failure_rate_pct = round((total_failures / total_tool_calls) * 100, 2) if total_tool_calls else 0.0
